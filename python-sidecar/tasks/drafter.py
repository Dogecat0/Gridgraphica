...
async def get_data_centers(facts: List, user_query: str) -> DataCenterList:
    ...
    template = "Extract all data center locations and infrastructure metrics for __QUERY__ from these facts:\n__FACTS__\n\nRequirements:\n- Coordinates must be decimal degrees.\n- id must be slug: TICKER-CITY-DATACENTER.\n- Extract itLoadMW, investment, squareFootage, pue, coolingTechnology, certifications, status, and ownershipType if available.\\n- For capacity (itLoadMW), extract the currently commissioned Critical IT Load. If only total planned capacity is available, note this in the status.\\n- For PUE, prefer the trailing 12-month operational average over the theoretical design PUE.\\n- investment should capture the total project cost (e.g., '$850M').\\n- squareFootage should capture the total building or campus floor area in sq ft.\\n- certifications should list known ratings (e.g., LEED Gold, Tier III).\\n- confidence must be 'verified' (Tier 1 source), 'unverified', 'unknown', or explicitly set to null.\n- GRANULARITY: Group individual buildings within the same city or campus into a single entry. However, DO NOT group distinct cities together into broad regional hubs (e.g., do not group City A and City B into a single 'Region Hub' - they must be separate).\n- EXHAUSTIVENESS METHODOLOGY: In the 'reasoning' field, you MUST first explicitly write out a complete list of EVERY single distinct city, county, parish, or campus you find in the facts. Once you have built this list in the reasoning field, you MUST create exactly one data center object in the 'dataCenters' array for each and every item in your list. Do not drop a single location."
    sys_prompt = f"Extract geographic data center data for {user_query}. MANDATE: Prioritize the parent company's data center footprint. Include major subsidiaries only if they are globally significant. NEVER use placeholders like 'Data Center A'; extract specific named sites when available, but you MUST also extract a city or region as a generic node (e.g. international locations like 'Frankfurt Data Centers') if no specific name is provided. Your extraction MUST be exhaustive. You must extract EVERY distinct location mentioned in the facts. Do not skip any regions, campuses, or cities."
